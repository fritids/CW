		...
		if ($smooth_slider['image_only'] == '1') { 
		   ...
		   if($permalink!='') {
			$html .= '<h2 '.$smooth_slider_css['smooth_slider_h2'].'><a '.$smooth_slider_css['smooth_slider_h2_a'].' href="'.$permalink.'">'.$post_title.'</a></h2><span '.$smooth_slider_css['smooth_slider_span'].'> '.$slider_excerpt.'</span>
				<p class="smooth_more"><a class="simplemodal-login" href="'.$permalink.'" '.$smooth_slider_css['smooth_slider_p_more'].'>'.$smooth_slider['more'].'</a></p>
...
function carousel_posts_on_slider_category($max_posts='5', $catg_slug='', $offset=0, $out_echo = '1') {
	...
	if ($loop) {
		$posts = array();
		$max_posts = $max_posts / count($catg_slug);
		foreach ($catg_slug as $cat) {
			$posts = array_merge($posts, get_posts('numberposts='.$max_posts.'&offset='.$offset.'&category='.$cat.$orderby));
		}
	} else {
		//extract posts
...
function get_smooth_slider_category($catg_slug,$offset=0, $limit = 10) {
	global $smooth_slider; 
	$r_array = carousel_posts_on_slider_category($limit, $catg_slug, $offset, '0'); 
...
function smooth_get_inline_css($echo='0'){
	...
	//smooth_slider_thumbnail
		$smooth_slider_css['smooth_slider_thumbnail']=$style_start.'float:'.$smooth_slider['img_align'].';margin:0 '.$thumb_margin_right.'px 0 '.$thumb_margin_left.'px;max-height:'.$smooth_slider['img_height'].'px;'.$style_end;
