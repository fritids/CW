  ...
  if(isset($_POST['submitted'])){
    ...
    //Check to see if the honeypot captcha field was filled in
	 ...
	 edit_profile ($_POST);
			  ...
			  $body .= '<a href="'.get_option('siteurl').'/premium-member?iduser='.$userid[0]->userid.'">'.get_option('siteurl').'/premium-member?iduser='.$userid[0]->userid.'</a><br />';
 ...
 $langs=list_spoken_lang ();
  foreach ($langs as $lang) 
  {
   ...
   <label><?php _e('E-mail ') ?><span class="rouge">*</span><br /></label>
					  <input type="text" name="email" id="email" value="<?php if(isset($_POST['email']))  {echo $_POST['email'];} else {echo $user_profile->email;}?>" class="requiredField email" />			
  </p>
   <p>
   <label>
