...
add_filter( 'wp_nav_menu_items', 'cw_custom_menu_item', 10, 2 );
function cw_custom_menu_item ( $items, $args ) {
    $items .= '<li><a href="/wp-login.php?redirect_to=' . $_SERVER['REQUEST_URI'] . '" class="simplemodal-login">Log In</a></li>';
    return $items;
}

// add_filter('template', 'cw_user_theme');
// add_filter('stylesheet', 'cw_user_theme');
function cw_user_theme($template = '') {
	global $user_ID;
	if (is_user_logged_in()) {
		// $template = 'connectingworld';
...
// add_action('init', 'cw_init_session', 1);
// function cw_init_session() {
//   session_start();
// }